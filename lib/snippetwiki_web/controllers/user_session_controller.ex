defmodule SnippetwikiWeb.UserSessionController do
  ...
  defp create(conn, %{"user" => %{"token" => token} = user_params}, info) do
    case Snippets.login_user_by_magic_link(token) do
  ...
  defp create(conn, %{"user" => user_params}, info) do
    ...
    if user = Snippets.get_user_by_email_and_password(email, password) do
      conn
      |> put_flash(:info, info)
      |> UserAuth.log_in_user(user, user_params)
    else
