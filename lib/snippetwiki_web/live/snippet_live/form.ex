defmodule SnippetwikiWeb.SnippetLive.Form do
  ...
  defp save_snippet(socket, :edit, snippet_params) do
    case Snippets.update_snippet(socket.assigns.current_scope, socket.assigns.snippet, snippet_params) do
      {:ok, snippet} ->
        {:noreply,
         socket
         |> put_flash(:info, "Snippet updated successfully")
         |> push_navigate(
           to: return_path(socket.assigns.current_scope, socket.assigns.return_to, snippet)
         )}
  ...
  defp return_path(_scope, "index", _snippet), do: ~p"/snippets"
  defp return_path(_scope, "show", snippet), do: ~p"/snippets/#{snippet}"
end
