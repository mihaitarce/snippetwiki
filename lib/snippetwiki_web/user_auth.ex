defmodule SnippetwikiWeb.UserAuth do
  use SnippetwikiWeb, :verified_routes
  ...
  @remember_me_cookie "_snippetwiki_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    user_token && Snippets.delete_user_session_token(user_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      SnippetwikiWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      SnippetwikiWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule SnippetwikiWeb.PageLive do
        use SnippetwikiWeb, :live_view

        on_mount {SnippetwikiWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SnippetwikiWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Snippets.sudo_mode?(socket.assigns.current_scope.user, -10) do
      {:cont, socket}
    else
      socket =
        socket
  ...
  def signed_in_path(%Plug.Conn{assigns: %{current_scope: %Scope{user: %Snippets.User{}}}}) do
    ~p"/users/settings"
  end
